...
import { CircleHelp, Pencil, Plus, Shield, Trash2 } from 'lucide-react';
import { apiClient } from '../api';
import { useProjectMeta, useToast } from '../context';
import { errorText } from '../lib/format';
import { memberRoleLabel, memberRoleTone } from '../lib/labels';
import { Modal } from '../components/shared/Modal';
import { PageHeader } from '../components/shared/PageHeader';
import { EmptyState } from '../components/shared/States';

/** 成员管理页：/projects/:projectId/members */
export function MembersView() {
  const { projectId, members, isOwner, refresh } = useProjectMeta();
    ...
    <>
      ...
      <section className="panel members-page">
        {members.length ? (
              ...
              <tbody>
                {members.map((member) => (
                  <tr key={member.id}>
                    ...
                    <td>
                      <span className={`role-tag ${memberRoleTone[member.role] || 'member'}`}>
          ...
          <EmptyState title="暂无成员" copy="项目成员信息加载后会显示在这里。" />
        )}
        {!isOwner && (
          <p className="permission-note">
            <Shield size={14} /> 你是项目成员，只能查看成员列表。
          </p>
        )}
      </section>
      {open && (
        <Modal title="添加成员" close={() => setOpen(false)}>
          <form className="form-stack" onSubmit={submit}>
            <label>
              用户标识
              <input name="user_id" required placeholder="user-123" />
            </label>
            <label>
              姓名
            ...
            <label>
              邮箱
              <input name="email" type="email" required placeholder="zhangsan@example.com" />
            </label>
            <label>
              角色
      ...
      {editMember && (
        ...
        >
          <form className="form-stack" onSubmit={updateRole}>
            ...
            <p className="permission-note">
              <CircleHelp size={14} /> 项目负责人可以管理成员和修改设置，成员可以管理任务和迭代，观察者只能查看。
            </p>
            <button className="primary-btn full" disabled={saving}>
              {saving ? '保存中…' : '保存角色'}
            </button>
