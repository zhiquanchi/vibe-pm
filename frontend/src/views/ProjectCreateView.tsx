          ...
          <form className="form-stack" onSubmit={submit}>
            ...
            <div className="stage-edit-list">
              ...
              {stages.map((stage, index) => (
                <div className="stage-edit-row" key={index}>
              ...
              <button
                type="button"
                className="ghost-btn"
                onClick={() => setStages((items) => [...items, { name: '' }])}
              >
                <Plus size={14} /> 添加阶段
              </button>
            </div>
            {validation && (
              <p className="permission-note">
                <CircleHelp size={14} /> {validation}
              </p>
            )}
            <button className="primary-btn full" disabled={saving || !!validation}>
              {saving ? '创建中…' : '创建项目'}
            </button>
          </form>
        </section>
