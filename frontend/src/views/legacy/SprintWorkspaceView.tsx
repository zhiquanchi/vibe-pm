...
import { Activity, CalendarDays, ChevronDown, RefreshCw } from 'lucide-react';
...
import { sprintStatusLabel, taskStatusWeight } from '../../lib/labels';
import type { ScopeChange, Sprint, SprintStatus, Task, TaskCreateInput } from '../../types';

/** 兼容视图：迭代工作台（看板 + 燃尽图 + 范围时间线）。 */
export function SprintWorkspaceView() {
  const { projectId, refresh, addNotice } = useProjectMeta();
  const { showToast } = useToast();
  const navigate = useNavigate();
  const params = useParams();
  const sprintId = params.sprintId ? Number(params.sprintId) : null;
  const workspace = useSprintWorkspace(sprintId);
  const sprint: Sprint | null = workspace.sprint;
  ...
  const total =
    sprint?.initial_points ||
    workspace.tasks.reduce((sum, task) => sum + task.story_points, 0);
  const completed = workspace.tasks.reduce(
  ...
  const runStatus = async () => {
    ...
    try {
      ...
      await refresh();
      showToast(statusModal === 'active' ? '迭代已开始' : '迭代已结束');
      setStatusModal(null);
    } catch (error) {
  ...
  return (
    <>
      <PageHeader
        eyebrow={`${sprint.name} · ${sprintStatusLabel[sprint.status]}`}
          ...
          <>
            <button
              ...
              onClick={() =>
                sprint.status === 'planning'
                  ? setDateModal(true)
                  : showToast(`${sprintStatusLabel[sprint.status]}迭代日期只读`)
              }
            >
              <CalendarDays size={15} /> {formatRange(sprint)} <ChevronDown size={14} />
            </button>
            {sprint.status !== 'completed' && (
              <button
                ...
                onClick={() =>
                  setStatusModal(sprint.status === 'planning' ? 'active' : 'completed')
                }
              >
                {workspace.mutationLoading
                  ? '处理中…'
                  : sprint.status === 'planning'
                    ? '开始迭代'
                    : '结束迭代'}
      ...
      <section className="metrics">
        ...
        <Metric
          ...
          note={`${displayChanges.reduce((sum, item) => sum + item.points_delta, 0) >= 0 ? '+' : ''}${displayChanges.reduce((sum, item) => sum + item.points_delta, 0)} pt`}
          tone="purple"
        />
      </section>
      <section className="grid-main">
        ...
        <ScopeTimeline
          ...
          capacityWarning={
            currentScope > total * 1.2
              ? `范围已增加 ${(currentScope - total).toFixed(0)} pt，当前容量可能不足`
              : null
          }
          onAddTask={() => showToast('请使用看板中的“新建任务”添加需求')}
          onSelectChange={setSelectedChange}
        />
      </section>
      <Board
      ...
      {statusModal && (
        <Modal
          title={statusModal === 'active' ? '开始迭代' : '结束迭代'}
          close={() => setStatusModal(null)}
        >
          <div className="confirm-copy">
            <b>
              {statusModal === 'active' ? '确认开始这次迭代？' : '确认结束这次迭代？'}
            </b>
            <p>
              {statusModal === 'active'
                ? `初始范围 ${workspace.tasks.reduce((sum, task) => sum + task.story_points, 0)} pt，共 ${workspace.tasks.length} 个任务。`
                : `当前完成率 ${currentScope ? Math.round((completed / currentScope) * 100) : 0}%，${workspace.tasks.filter((task) => task.status !== 'done').length} 个未完成任务将回到 Backlog。`}
            </p>
          </div>
          <button
      ...
      <div className="form-grid">
        <label>
          ...
          <input
            name="start_date"
            type="date"
            defaultValue={sprint.start_date.slice(0, 10)}
            required
          />
        </label>
        <label>
          ...
          <input
            name="end_date"
            type="date"
            defaultValue={sprint.end_date.slice(0, 10)}
            required
          />
        </label>
      </div>
