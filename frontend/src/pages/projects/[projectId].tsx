...
import { statusLabel, statusTone, formatDate, formatRange, sprintPath } from '@/utils/format';
...
export default function ProjectOverviewPage() {
  ...
  useEffect(() => {
    void apiClient.listBacklog(projectId).then(setBacklog).catch(() => undefined);
  }, [projectId, sprint?.id]);
      ...
      <PageHeader
        ...
        actions={
          <button className="primary-btn" onClick={() => navigate(`/projects/${projectId}/sprints`)}>
            <DotChartOutlined style={{ fontSize: 15 }} /> 查看迭代
          </button>
        }
      />
      {resource.error ? (
        <ErrorState message={resource.error} retry={onRefresh} />
      ) : (
        <>
          <section className="metrics">
            <Metric
              ...
              note={sprint ? `${formatRange(sprint)} · ${statusLabel[sprint.status]}` : '先创建或开始一个迭代'}
              tone="blue"
            />
            <Metric
              label="迭代范围"
          ...
          <div className="overview-grid">
            <section className="panel overview-section">
              <div className="panel-head">
                ...
                {sprint && <span className={`status-pill ${statusTone[sprint.status]}`}>{statusLabel[sprint.status]}</span>}
              </div>
              {sprint ? (
                <>
                ...
                <EmptyState
                  ...
                  copy={sprints.length ? '请先开始一个已规划的迭代。' : '新建一个迭代后，项目状态会显示在这里。'}
                  action={
                    <button className="primary-btn" onClick={() => navigate(`/projects/${projectId}/sprints`)}>
                      <PlusOutlined style={{ fontSize: 15 }} /> {sprints.length ? '查看迭代' : '新建迭代'}
                    </button>
                  }
                />
              )}
            ...
            <section className="panel overview-section">
              <div className="panel-head">
                ...
                <button className="text-btn" onClick={() => navigate(`/projects/${projectId}/reports/${sprint?.id || ''}`)}>
                  查看报告
                </button>
              </div>
              {recent.length ? (
                <div className="activity-list">
                  {recent.map((change) => (
                    <button
                      ...
                      onClick={() => navigate(`${sprintPath(projectId, change.sprint_id)}?change_id=${change.id}`)}
                    >
                      <span className={`notice-dot ${change.points_delta >= 0 ? 'scope' : 'end'}`} />
                      <span>
                        <b>{change.description}</b>
                        <small>{formatDate(change.created_at)} · {change.reason || '未填写原因'}</small>
                      </span>
