...
import { formatDate, errorText } from '@/utils/format';
...
export default function BacklogPage() {
  ...
  const currentSprint = ctx.currentSprint;
  ...
  const load = () => apiClient.listBacklog(projectId).then(setTasks).catch((error) => onToast(errorText(error)));
  useEffect(() => {
    void load();
  }, [projectId]);
  ...
  const addSelected = async () => {
    ...
    if (!target) {
      onToast('请先新建一个规划中的迭代');
      return;
    }
    try {
      for (const id of selected) await apiClient.addTaskToSprint(target.id, id, '从 Backlog 规划加入');
      setSelected([]);
      await load();
      await onRefresh();
      onToast(`已将 ${selected.length} 个任务加入 ${target.name}`);
    } catch (error) {
      ...
      <PageHeader
        ...
        actions={
          <button className="primary-btn" onClick={() => setOpen(true)}>
            <PlusOutlined style={{ fontSize: 15 }} /> 创建任务
          </button>
        }
      />
      <section className="panel backlog-page">
        <div className="filter-bar">
          <label className="search">
            ...
            <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="按标题搜索" />
          </label>
          ...
          <button className="ghost-btn" disabled={!selected.length} onClick={() => void addSelected()}>
            <InboxOutlined style={{ fontSize: 14 }} /> 加入规划中的迭代 ({selected.length})
          </button>
        </div>
        {filtered.length ? (
              ...
              <tbody>
                {filtered.map((task) => (
                  <tr key={task.id}>
                    ...
                    <td>
                      <span className={`priority p-${task.priority}`}>{task.priority}</span>
                    </td>
      ...
      {open && (
        <Modal title="创建 Backlog 任务" close={() => setOpen(false)}>
          <form className="form-stack" onSubmit={create}>
            <label>
              任务标题<input name="title" required />
            </label>
            ...
            <label>
              负责人<input name="assignee" placeholder="姓名或缩写" />
            </label>
