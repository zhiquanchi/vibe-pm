...
import type { ProjectMember, Stage, StageDeletePreview, StageStatus } from '@/types';
...
export default function StageListPage() {
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const stageList = await apiClient.listStages(projectId);
      setStages(stageList);
    } catch (err) {
  ...
  const move = (stage: Stage, delta: number) => {
    ...
    void run(() => apiClient.reorderStages(projectId, ids), '阶段顺序已调整');
  };
  const remove = async (stage: Stage) => {
    try {
      await apiClient.deleteStage(projectId, stage.id);
      await load();
      onToast('阶段已删除');
    } catch (err) {
      const detail = err instanceof ApiError ? (err.body as { detail?: unknown } | null)?.detail : null;
      if (err instanceof ApiError && err.status === 409 && detail && typeof detail === 'object' && 'confirm_required' in detail) {
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <button className="primary-btn" disabled={!isOwner} title={!isOwner ? '只有项目负责人可以修改阶段结构' : undefined} onClick={() => setModal({ kind: 'add' })}>
            <PlusOutlined style={{ fontSize: 15 }} /> 新增阶段
          </button>
        }
      />
      {error ? (
        ...
        <div className="state-panel">
          <SafetyCertificateOutlined className="spin" style={{ fontSize: 20 }} />
          <b>正在加载项目数据…</b>
        </div>
      ) : !stages.length ? (
        <EmptyState
          ...
          {stages.map((stage) => (
            <div className={`stage-row ${stage.status}`} key={stage.id}>
              ...
              <div className="stage-meta">
                ...
                <small>{stage.planned_start || stage.planned_end ? `${formatDate(stage.planned_start)} - ${formatDate(stage.planned_end)}` : '未排期'}</small>
              </div>
              ...
              <div className="stage-row-actions">
                ...
                {isOwner && stage.status === 'planned' && (
                  <button className="ghost-btn small" onClick={() => setModal({ kind: 'start', stage })}>
                    <CaretRightOutlined style={{ fontSize: 13 }} /> 启动
                  </button>
                )}
                {isOwner && stage.status === 'active' && !stage.is_primary && (
                  <button className="ghost-btn small" onClick={() => void run(() => apiClient.setPrimaryStage(projectId, stage.id), '主阶段已切换')}>
                    <FlagOutlined style={{ fontSize: 13 }} /> 设为主阶段
                  </button>
                )}
                {isOwner && stage.status === 'active' && (
                  <button className="ghost-btn small" onClick={() => setModal({ kind: 'complete', stage })}>
                    <CheckOutlined style={{ fontSize: 13 }} /> 完成
                  </button>
                )}
                {isOwner && (
                  <button className="icon-btn" title="编辑阶段" onClick={() => setModal({ kind: 'edit', stage })}>
                    <EditOutlined style={{ fontSize: 14 }} />
                  </button>
                )}
                {isOwner && stage.status !== 'completed' && (
                  <button className="icon-btn danger" title="删除阶段" onClick={() => void remove(stage)}>
      ...
      {!isOwner && !loading && !error && (
        <p className="permission-note">
          <SafetyCertificateOutlined style={{ fontSize: 14 }} /> 你是项目成员，只能查看阶段列表。
        </p>
      )}
      {modal && (modal.kind === 'add' || modal.kind === 'edit') && (
        <StageFormModal
          ...
          onSaved={(message) => void run(async () => undefined, message)}
          reload={load}
          onToast={onToast}
        />
      )}
      {modal?.kind === 'delete' && (
        <Modal title="删除阶段" close={() => setModal(null)}>
          <div className="form-stack">
            <p>
              删除「{modal.stage.name}」将同时影响其中 <b>{modal.impact.tasks}</b> 个任务和 <b>{modal.impact.deliverables}</b> 个交付物，确认删除？
            </p>
            <div className="form-grid">
              <button className="ghost-btn" onClick={() => setModal(null)}>
                取消
              </button>
              <button className="primary-btn danger" onClick={() => void run(() => apiClient.deleteStage(projectId, modal.stage.id, true), '阶段已删除')}>
                确认删除
              </button>
            </div>
          </div>
      ...
      {modal?.kind === 'start' && (
        <Modal title={`启动阶段「${modal.stage.name}」`} close={() => setModal(null)}>
          <div className="form-stack">
            ...
            <div className="form-grid">
              <button className="ghost-btn" disabled={!activeStages.length} onClick={() => void run(() => apiClient.startStage(projectId, modal.stage.id, false), '阶段已启动为并行阶段')}>
                并行启动
              </button>
              <button className="primary-btn" onClick={() => void run(() => apiClient.startStage(projectId, modal.stage.id, true), '阶段已启动')}>
      ...
      {modal?.kind === 'complete' && (
        <StageCompleteModal
          ...
          onClose={() => setModal(null)}
          onDone={(message) => void run(async () => undefined, message)}
          onToast={onToast}
        />
      )}
    </>
...
function StageFormModal({
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (stage) await apiClient.updateStage(projectId, stage.id, input);
      else await apiClient.addStage(projectId, input);
      await reload();
      onClose();
  ...
  return (
    <Modal title={stage ? `编辑阶段「${stage.name}」` : '新增阶段'} close={onClose}>
      <form className="form-stack" onSubmit={submit}>
        <label>
          阶段名称
          <input name="name" defaultValue={stage?.name || ''} required disabled={completed} title={completed ? '已完成阶段不能重命名' : undefined} />
        </label>
        <label>
          阶段目标
        ...
        <div className="form-grid">
          <label>
            计划开始
            <input name="planned_start" type="date" defaultValue={stage?.planned_start || ''} />
          </label>
          <label>
            计划结束
...
function StageCompleteModal({
  ...
  const submit = async () => {
    ...
    setSaving(true);
    try {
      await apiClient.completeStage(projectId, stage.id, successor ? Number(successor) : undefined);
      onClose();
      onDone('阶段已完成');
    } catch (err) {
      onToast(errorText(err));
