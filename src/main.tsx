...
import type { ScopeChange, SprintStatus, Task, TaskCreateInput, TaskStatus } from './types';
import './styles.css';
...
function App() {
  const workspace = useSprintWorkspace(SPRINT_ID);
  ...
  const handleCreateTask = async (input: TaskCreateInput) => {
    await workspace.createTask({ ...input, project_id: 1, sprint_id: SPRINT_ID });
  ...
  return <div className="app-shell">
    ...
    <main className="main"><header className="topbar"><div className="breadcrumbs"><span>Vibe PM</span><span>/</span><b>{sprint?.name || 'Sprint'}</b><span className="status-pill active"><i /> {sprint ? statusLabel[sprint.status] : '加载中'}</span></div><div className="top-actions"><button className="icon-btn" title="通知"><Bell size={18} /></button><div className="avatar avatar-blue">XM</div></div></header>
      ...
      <section className="grid-main"><div className="chart-card panel">{workspace.loading && !workspace.snapshots.length ? <div className="loading-state">正在加载燃起图…</div> : <BurnupChart snapshots={workspace.snapshots} scopeChanges={workspace.scopeChanges} initialPoints={sprint?.initial_points} />}</div><ScopeTimeline changes={workspace.scopeChanges} capacityWarning={capacityWarning} onAddTask={() => void handleScopeAdd()} onSelectChange={setSelectedChange} /></section>
      <Board tasks={displayTasks} projectId={1} sprintId={SPRINT_ID} onCreateTask={handleCreateTask} onStatusChange={handleStatus} onEditTask={async (task, input) => { await workspace.updateTask(task.id, input); await workspace.refresh(); }} onDeleteTask={handleDelete} />
